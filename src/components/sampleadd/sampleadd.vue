<template>
    <div class="sampletypeadd">
        <div class="common-level-rail">
            <el-form size="small" :model="form" ref="form" v-if="sampletypes=='1'">
            <el-col :span="4">
          ...
          <el-form size="small" :model="form" ref="form" v-if="sampletypes=='3'">
            <el-col :span="8">
              <el-form-item label="样品" prop="sample_name" label-width="80px" :rules="[{required: true,message: '请选择样品',trigger: ['blur','change']}]">
                <el-select v-model="form.category_id" filterable placeholder="请选择样品">
                    <el-option v-for="cat in cateList" :value="cat.id" :key="cat.id" :label="cat.sample_name"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="样品名称" prop="sample_name" label-width="80px" :rules="[{required: true,message: '请输入型品名称',trigger: ['blur']}]">
          ...
          <el-table size="small" :data="tableData" :span-method="objectSpanMethod" border>
            ...
            <el-table-column label="实验标准" width="100">
              <template slot-scope="scope">
                ...
                <el-button v-if="sampletypes!='2'" size="small" type="text" @click="addType(scope.$index)">添加标准</el-button>
              </template>
            </el-table-column>
            <el-table-column label="A" width="100">
              <template slot-scope="scope">
                <el-checkbox v-model="scope.row.a" :disabled="sampletypes=='3'" @change="changeBox(scope.row,1)" false-label="0" true-label="1"></el-checkbox>
              </template>
            </el-table-column>
            <el-table-column label="B" width="100">
              <template slot-scope="scope">
                <el-checkbox v-model="scope.row.b" :disabled="sampletypes=='3'" @change="changeBox(scope.row,2)" false-label="0" true-label="1"></el-checkbox>
              </template>
            </el-table-column>
            <el-table-column label="C" width="100">
              <template slot-scope="scope">
                <el-checkbox v-model="scope.row.c" :disabled="sampletypes=='3'" @change="changeBox(scope.row,3)" false-label="0" true-label="1"></el-checkbox>
              </template>
            </el-table-column>
            <el-table-column fixed="right" label="操作" width="90" v-if="sampletypes=='1'">
              <template slot-scope="scope">
...
import sampletype from "../sampletype/sampletype.vue"
export default{
    components:{
        sampletype
    },
    ...
    methods:{
      changeClass(){
       ...
       /**样品类型 */
       sampleShowSampleTypeList(){
        SampleApi.sampleShowSampleTypeList({}, true)
        .then(res=>{
          let {data} = res
      ...
      getData() {
        ...
        SampleApi.sampleEditShowSampleType(Params, true)
          .then(res => {
            ...
            data.list.data.forEach(item => {
              ...
              if(index==-1){
                self.product_arr.push(item.experiment_id);
              }
        ...
        onSubmit(){  
          ...
          self.$refs.form.validate((valid)=>{
            if (valid) {
              self.form.id = self.$route.query.cample_id || '';
