  ...
  <div class="sampletypeadd">
      <div class="common-level-rail">
        <el-form size="small" :model="form" ref="form">
            <el-col :span="8">
              <el-form-item label="样品" prop="samplelist_id" label-width="80px" :rules="[{required: true,message: '请选择样品',trigger: ['blur','change']}]">
                <el-select v-model="form.samplelist_id" filterable @change="changeClass" placeholder="请选择样品">
      ...
      <div class="pb50">
        <el-table size="small" :data="tableData" :span-method="objectSpanMethod" border>
          ...
          <el-table-column label="A" width="100">
            <template slot-scope="scope">
              <el-checkbox v-model="scope.row.a" disabled false-label="0" true-label="1"></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column label="B" width="100">
            <template slot-scope="scope">
              <el-checkbox v-model="scope.row.b" disabled false-label="0" true-label="1"></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column label="C" width="100">
            <template slot-scope="scope">
              <el-checkbox v-model="scope.row.c" disabled false-label="0" true-label="1"></el-checkbox>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
export default{
  ...
  methods:{
    ...
    /**样品类型 */
    sampleShowSampleListModel(){
      SampleApi.sampleShowSampleListModel({}, true)
      .then(res=>{
        let {data} = res
    ...
    init(){
      ...
      self.loading = true;
      SampleApi.sampleDditShowSampleModel(Params, true)
        .then(res => {
          self.loading = false;
          let {data} = res
    ...
    getData() {
      ...
      onSubmit(){  
        ...
        self.$refs.form.validate((valid)=>{
          if (valid) {
            self.form.samplemodel_id = self.$route.query.cample_id || '';
            self.form.sample_list = self.tableData;
            self.loading = true;
            SampleApi.sampleAddSampleModel(self.form, true)
            .then(res => {
              self.loading = false;
              if(res.code==1){
                self.$message({
                ...
                self.$router.push('/sample/samplemodel/index');
              }else{
                self.$error({
                  message: res.msg,
                  type: 'success'
